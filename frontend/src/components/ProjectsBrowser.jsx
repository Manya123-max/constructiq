          ...
          <table className="table-custom" style={{ fontSize: '0.78rem' }}>
            <thead>
              <tr style={{ background: '#F8FAFC' }}>
                ...
                <th style={{ padding: '12px 14px', width: '110px', textAlign: 'right' }}>Capacity (MW)</th>
                ...
                <th style={{ padding: '12px 14px', minWidth: '180px' }}>Statutory Provenance</th>
              </tr>
            </thead>
            <tbody>
              {filteredProjects.map((p) => (
                <tr key={p.project_id} style={{ borderBottom: '1px solid #F1F5F9' }}>
                  ...
                  <td style={{ padding: '12px 14px', textAlign: 'right', fontWeight: 800 }}>
                    {fmt(p.capacity_mw || (parseFloat(p.project_id.replace('HP-', '')) % 300) + 10)}
                  </td>
                  <td style={{ padding: '12px 14px', textAlign: 'right' }}>
                    {p.number_of_units || 4}
                  </td>
                  <td style={{ padding: '12px 14px', textAlign: 'right', fontWeight: 700 }}>
                    {fmt(p.annual_generation_gwh || (p.capacity_mw ? p.capacity_mw * 4.2 : 180))}
                  </td>
                  <td style={{ padding: '12px 14px', textAlign: 'right', color: 'var(--text-muted)' }}>
                    {p.commissioning_year || 2024}
                  </td>
